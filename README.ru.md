...
Математика проекции откалибрована против реального рендера, чтобы метки ложились на пиксели. Маркеры, расставленные по кадру, проецируются с горизонтальной точностью до пары пикселей относительно того, где их рисует Movie Render Queue:

![Спроецированные точки поверх рендера](docs/images/calibration.png)
